/* Ordaklok — felles app-state og hjelparar */
(function (root) {
  'use strict';
  ...
  // Sjekk om eit svar matchar (med valfrie alternative svar i pair.alts).
  function isAnswerCorrect(input, pair, direction) {
    const target = direction === 'BA' ? pair.a : pair.b;
    const targets = [target].concat(pair.alts || []);
    const normIn = normalizeAnswer(input);
    return targets.some(t => normalizeAnswer(t) === normIn);
  }

  // Direction: 'AB' = vis A spør om B (default), 'BA' = vis B spør om A.
  function getPrompt(pair, direction) {
    return direction === 'BA' ? pair.b : pair.a;
  }
  function getAnswer(pair, direction) {
    return direction === 'BA' ? pair.a : pair.b;
  }
  function getPromptLabel(list, direction) {
    return direction === 'BA' ? (list.labelB || 'Side B') : (list.labelA || 'Side A');
  }
  function getAnswerLabel(list, direction) {
    return direction === 'BA' ? (list.labelA || 'Side A') : (list.labelB || 'Side B');
  }
  ...
  // Lett-validering av lista (returnerer { ok, error, list })
  function validateList(obj) {
    if (!obj || typeof obj !== 'object') return { ok: false, error: 'Ikkje eit objekt.' };
    if (obj.app && obj.app !== 'ordaklok') return { ok: false, error: 'Lista er for "' + obj.app + '", ikkje Ordaklok.' };
    if (!Array.isArray(obj.pairs)) return { ok: false, error: 'Manglar feltet "pairs".' };
    const cleaned = {
      app: 'ordaklok',
      version: 1,
      id: obj.id || ('list_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8)),
      title: String(obj.title || 'Importert liste').slice(0, 200),
      labelA: String(obj.labelA || 'Side A').slice(0, 60),
      labelB: String(obj.labelB || 'Side B').slice(0, 60),
      description: String(obj.description || '').slice(0, 1000),
      pairs: obj.pairs
        .filter(p => p && typeof p === 'object')
        .map(p => {
          const out = { a: String(p.a || '').slice(0, 200), b: String(p.b || '').slice(0, 200) };
          if (Array.isArray(p.alts) && p.alts.length) {
            out.alts = p.alts.map(x => String(x).slice(0, 200)).filter(Boolean);
          }
          return out;
        })
        .filter(p => p.a && p.b),
    ...
    if (cleaned.pairs.length === 0) return { ok: false, error: 'Lista har ingen gyldige par.' };
    return { ok: true, list: cleaned };
  }

  // Lucide-ikon som SVG-strenger
  const ICONS = {
    ...
    x: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" x2="6" y1="6" y2="18"/><line x1="6" x2="18" y1="6" y2="18"/></svg>',
    ...
    refreshCcw: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M3 21v-5h5"/></svg>',
    ...
    repeat: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m17 2 4 4-4 4"/><path d="M3 11v-1a4 4 0 0 1 4-4h14"/><path d="m7 22-4-4 4-4"/><path d="M21 13v1a4 4 0 0 1-4 4H3"/></svg>'
  };
